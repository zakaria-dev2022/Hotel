...
namespace Hotel
{
    internal class Utils
    {
        ...
        public static MySqlConnection cnx = new MySqlConnection("server=localhost;database=hotel;uid=root;password=");
        ...
        public static void AfficherImageAdmin(PictureBox pictureBox)
        {
            try
            {
                ...
                MySqlCommand command = new MySqlCommand(query, cnx);
                command.Parameters.AddWithValue("@Id", 2); // Remplacer 1 par l'ID de l'admin dont vous voulez afficher l'image

                // Récupérer le chemin de l'image à partir de la base de données
                string imagePath = (string)command.ExecuteScalar();

                pictureBox.Load(@"C:\laragon\www\hotel\img\logo\" + imagePath);
